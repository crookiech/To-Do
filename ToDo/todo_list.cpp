...
void ToDo_List::on_delete_task_clicked()
{
    ...
    if (selectedItem) {
        ...
        qDebug() << "Deleting task with ID:" << task.task_id << "Title:" << task.title;
...
void ToDo_List::load_tasks_from_file(const QString &filePath)
{
    ...
    if (file_manager.load_tasks_from_file(filePath, tasks)) {
        ...
        for (const auto &task : tasks) {
            bool userMatches = (task.user_id == current_user_id);
        ...
        qDebug() << "todolist.cpp: error loading tasks from JSON file";
    }
}
...
void ToDo_List::on_list_tasks_changed(QListWidgetItem *item)
{
    ...
    qDebug() << "Updating task with ID:" << task.task_id << "Checked:" << task.checked;
    if (!file_manager.update_task_in_file(path_to_tasks, task)) {
        qDebug() << "todolist.cpp: failed to update the task in JSON file";
